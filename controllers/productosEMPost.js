...
module.exports = async (req, res) => {
    for (a = 0; a < req.body['MaterialesProductos[cantidad]'].length; a++) {
        await Producto.updateOne(
            { nombre: req.body.ProductoAEditar },
            {
                $set: {
                    'MaterialesProductos.$[item]': {
                        Descripcion: req.body['MaterialesProductos[nombre]'][a],
                        cantidad: req.body['MaterialesProductos[cantidad]'][a],
                        codigo: req.body['MaterialesProductos[codigo]'][a],
                        familia: req.body['MaterialesProductos[familia]'][a],
                    },
                },
            },
            {
                arrayFilters: [
                    {
                        'item.codigo':
                            req.body['MaterialesProductos[codigo]'][a],
                    },
                ],
            }
        );
    ...
    for (b = 0; b < req.body['PinturaProductos[cantidad]'].length; b++) {
        await Producto.updateOne(
            { nombre: req.body.ProductoAEditar },
            {
                $set: {
                    'PinturaProductos.$[item]': {
                        Descripcion: req.body['PinturaProductos[nombre]'][b],
            ...
            {
                arrayFilters: [
                    { 'item.codigo': req.body['PinturaProductos[codigo]'][b] },
                ],
            }
        );
    }
            ...
            { nombre: req.body.ProductoAEditar },
            {
                $set: {
                    'InstalacionProductos.$[item]': {
                        ...
                        cantidad: req.body['InstalacionProductos[cantidad]'][c],
                        codigo: req.body['InstalacionProductos[codigo]'][c],
                        familia: req.body['InstalacionProductos[familia]'][c],
                    },
                },
            },
            {
                arrayFilters: [
                    {
                        'item.codigo':
                            req.body['InstalacionProductos[codigo]'][c],
                    },
                ],
            }
        );
    ...
    for (let a = 0; a < productos.length; a++) {
        ...
        for (let i = 0; i < PinturaProductos.length; i++) {
            if (
                PinturaProductos[i].Descripcion === materiales[i].Descripcion &&
                materiales[i].PrecioUnitario >= 0
            ) {
                sumaSolventes =
                    sumaSolventes +
                    PinturaProductos[i].cantidad * materiales[i].PrecioUnitario;
            }
        }
