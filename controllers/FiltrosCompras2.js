    ...
    if (req.session?.passport?.user != undefined) {
        ...
        if (page === undefined) {
            ...
            if (Filtro === 'NombreProducto') {
                ...
                const compra = await Compra.paginate(
                    {
                        ProductosComprados: {
                            $elemMatch: {
                                nombre: { $regex: req.body.busqueda },
                            },
                        },
                    },
                    { page: 1, limit: 30 }
                ...
                for (b = 0; b < compra.docs.length; b++) {
                    ...
                    for (a = 0; a < ProductosComprados.length; a++) {
                        suma =
                            ProductosComprados[a].precio *
                            ProductosComprados[a].cantidad;
                        SubTotal.push(suma);
                    }
                }
            ...
            if (Filtro === 'IdTrans') {
                ...
                const { años } = añocreado[0];
                const compra = await Compra.paginate(
                    { Id_transaccion: { $regex: req.body.busqueda } },
                    { page: 1, limit: 30 }
                );
                var SubTotal = [];
                var Total = 0;
            ...
            if (Filtro === 'NombreCliente') {
                ...
                const { años } = añocreado[0];
                const compra = await Compra.paginate(
                    { Nombre_comprador: { $regex: req.body.busqueda } },
                    { page: 1, limit: 30 }
                );
                var SubTotal = [];
                var Total = 0;
                ...
                const compra = await Compra.paginate(
                    {
                        ProductosComprados: {
                            $elemMatch: {
                                precio: {
                                    $gt: Number(req.body.BusquedaPrecios),
                                    $lt: Number(req.body.BusquedaPrecios2),
                                },
                            },
                        },
                    },
            ...
            if (Filtro === 'NombreProducto') {
                ...
                const compra = await Compra.paginate(
                    {
                        ProductosComprados: {
                            $elemMatch: { nombre: { $regex: consulta } },
                        },
                    },
                    { page, limit: 30 }
                );
            ...
            if (Filtro === 'NombreCliente') {
                ...
                const { años } = añocreado[0];
                const compra = await Compra.paginate(
                    { Nombre_comprador: { $regex: consulta } },
                    { page, limit: 30 }
                );
                var SubTotal = [];
                var Total = 0;
