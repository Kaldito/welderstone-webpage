...
module.exports = async (req, res) => {
  ...
  try {
    // Obtener el producto con populate para los materiales
    const producto = await Product.findOne({ Codigo: codigo }).populate("MaterialesProductos.material");
    if (!producto) {
    ...
    const imagen = image || "/images/productos/PorDefecto.jpg";

    // Preparar datos de materiales
    const materiales = MaterialesProductos.map((mp) => ({
      codigo: mp.material.Codigo,
      descripcion: mp.material.Descripcion,
      unidad: mp.material.Unidad,
      cantidad: mp.cantidad,
      precioUnitario: mp.material.PrecioUnitario,
      importe: mp.cantidad * mp.material.PrecioUnitario,
      familia: mp.material.Familia,
    }));


    // Calcular totales
    const sumaMateriales = materiales.reduce((acc, curr) => acc + curr.importe, 0);
    let total = sumaMateriales;
    const manoObra = (ManoObGeneral * total) / 100;
    total += manoObra;
    const herramientaMenor = (manoObra * HerramientaMenor) / 100;
    total += herramientaMenor;
    const indirectos = (total * PorcentajeGeneral) / 100;
    total += indirectos;
    const subtotal = total;
    const ivaCalculo = (iva * subtotal) / 100;
    const totalFinal = subtotal + ivaCalculo;
    ...
    const materialesFormateados = materiales.map((m) => ({
      ...m,
      precioUnitario: formatNumber(m.precioUnitario),
      importe: formatNumber(m.importe),
    }));

    const sumaMaterialesFormateado = formatNumber(sumaMateriales);
    const manoObraFormateado = formatNumber(manoObra);
    const herramientaMenorFormateado = formatNumber(herramientaMenor);
    const indirectosFormateado = formatNumber(indirectos);
    const subtotalFormateado = formatNumber(subtotal);
    const ivaCalculoFormateado = formatNumber(ivaCalculo);
    const totalFinalFormateado = formatNumber(totalFinal);

    // Fechas
    const fechaCompra = new Date();
    const date = `${fechaCompra.getFullYear()}-${fechaCompra.getMonth() + 1}-${fechaCompra.getDate()}`;
    const caduca = `${fechaCompra.getFullYear()}-${fechaCompra.getMonth() + 2}-${fechaCompra.getDate()}`;

    // Obtener imagen como data URL
    const imageDataURL = await getImageDataUrl(imagen);
    ...
    const docDefinition = {
      content: [
        {
          columns: [
            { image: "./public/images/productos/welderstone.png", width: 160, alignment: "left" },
            {},
            { image: imageDataURL, width: 80, height: 80, alignment: "center", fit: [230, 230], absolutePosition: { x: 50, y: 10 } },
        ...
        },
        {
          ...
          table: {
            widths: [80, 80, 80],
            body: [
              [{ text: "Fecha", style: "tableHeader", alignment: "left" }, { text: date, alignment: "left" }],
              [{ text: "Codigo", style: "tableHeader", alignment: "left" }, { text: codigo, alignment: "left" }],
              [{ text: "Valido hasta", style: "tableHeader", alignment: "left" }, { text: caduca, alignment: "left" }],
            ],
          },
        ...
        { text: [{ text: "Descripción del producto\n", style: "redText" }, descripcion], margin: [0, 20] },
        { alignment: "center", columns: [{ width: 300, text: "", alignment: "center" }] },
        { text: "\n" },
        {
          style: "tableExample",
          alignment: "center",
          table: {
            ...
            body: [
              ...
              ...materialesFormateados.map((m, i) => [
                { fillColor: i % 2 === 0 ? "#FFFFFF" : "#F2F2F2", fontSize: 10, text: m.codigo },
                { fillColor: i % 2 === 0 ? "#FFFFFF" : "#F2F2F2", fontSize: 10, text: m.descripcion, alignment: "left" },
                { fillColor: i % 2 === 0 ? "#FFFFFF" : "#F2F2F2", fontSize: 10, text: m.unidad },
                { fillColor: i % 2 === 0 ? "#FFFFFF" : "#F2F2F2", fontSize: 10, text: m.cantidad },
                { fillColor: i % 2 === 0 ? "#FFFFFF" : "#F2F2F2", fontSize: 10, text: `$${m.precioUnitario}` },
                { fillColor: i % 2 === 0 ? "#FFFFFF" : "#F2F2F2", fontSize: 10, text: `$${m.importe}` },
              ]),
              ...
              [{ fillColor: "#F2F2F2", text: " " }, { fillColor: "#F2F2F2", text: " " }, { fillColor: "#F2F2F2", text: "Material", bold: true }, { fillColor: "#F2F2F2", text: " ", bold: true }, { fillColor: "#F2F2F2", text: "Suma", bold: true }, { fillColor: "#F2F2F2", text: `$${sumaMaterialesFormateado}`, bold: true }],
              [{ fillColor: "#F2F2F2", text: " " }, { fillColor: "#F2F2F2", text: " " }, { fillColor: "#F2F2F2", text: "Mano de obra", bold: true }, { fillColor: "#F2F2F2", text: `${ManoObGeneral}%`, bold: true }, { fillColor: "#F2F2F2", text: `$${sumaMaterialesFormateado}`, bold: true }, { fillColor: "#F2F2F2", text: `$${manoObraFormateado}`, bold: true }],
              [{ fillColor: "#F2F2F2", text: " " }, { fillColor: "#F2F2F2", text: " " }, { fillColor: "#F2F2F2", text: "Herramienta menor", bold: true }, { fillColor: "#F2F2F2", text: `${HerramientaMenor}%`, bold: true }, { fillColor: "#F2F2F2", text: `$${manoObraFormateado}`, bold: true }, { fillColor: "#F2F2F2", text: `$${herramientaMenorFormateado}`, bold: true }],
              [{ fillColor: "#F2F2F2", text: " " }, { fillColor: "#F2F2F2", text: " " }, { fillColor: "#F2F2F2", text: "Indirectos + Honorarios", bold: true }, { fillColor: "#F2F2F2", text: `${PorcentajeGeneral}%`, bold: true }, { fillColor: "#F2F2F2", text: `$${formatNumber(subtotal - indirectos)}`, bold: true }, { fillColor: "#F2F2F2", text: `$${indirectosFormateado}`, bold: true }],
              [{ fillColor: "#FFFFFF", text: " " }, " ", " ", " ", { fillColor: "#FFFFFF", text: "SubTotal", bold: true }, { fillColor: "#FFFFFF", text: `$${subtotalFormateado}`, bold: true }],
              [{ fillColor: "#FFFFFF", text: " " }, " ", " ", " ", { fillColor: "#FFFFFF", text: "Iva", bold: true }, { fillColor: "#FFFFFF", text: `$${ivaCalculoFormateado}`, bold: true }],
              [{ fillColor: "#FFFFFF", text: " " }, " ", " ", " ", { fillColor: "#FFFFFF", text: "Total", bold: true }, { fillColor: "#FFFFFF", text: `$${totalFinalFormateado}`, bold: true }],
              [{ fillColor: "#F2F2F2", text: " " }, { fillColor: "#F2F2F2", text: " " }, { fillColor: "#F2F2F2", text: " " }, { fillColor: "#F2F2F2", text: " " }, { fillColor: "#F2F2F2", text: " " }, { fillColor: "#F2F2F2", text: " " }],
            ],
          },
          layout: {
      ...
      pageSize: { width: 800, height: 1230 },
      styles: {
        redText: { color: "red" },
        header: { fontSize: 18, bold: true, margin: [0, 0, 0, 10] },
        subheader: { fontSize: 16, bold: true, margin: [0, 10, 0, 5] },
        tableExample: { margin: [0, 5, 0, 15], alignment: "center" },
        tableHeader: { bold: true, fontSize: 13, color: "black", alignment: "center" },
      },
      defaultStyle: { columnGap: 15 },
    };

    // Generar y subir el PDF a S3
    const printer = new PdfPrinter(fonts);
    const pdfDoc = printer.createPdfKitDocument(docDefinition);
    const fileName = `pdfs/${IdTransaccion}${codigo}.pdf`;
    const chunks = [];
    ...
    pdfDoc.on("end", () => {
      ...
      const bucketName = "welderstonebucket";
      const s3Params = { Bucket: bucketName, Key: fileName, Body: pdfBuffer };
      s3.upload(s3Params, (err, data) => {
        if (err) {
          console.error("Error uploading PDF to S3:", err);
          return res.status(500).send("Failed to upload PDF to S3");
        }
    ...
    // Renderizar la vista
    const role = req.session?.passport?.user?.role || "viewer";
    ...
    res.render("pdfProducto", {
        ...
        materialesFormateados, // Asegúrate de pasar este arreglo completo
        sumaMaterialesFormateado,
        manoObraFormateado,
        herramientaMenorFormateado,
        indirectosFormateado,
        subtotalFormateado,
        ivaCalculoFormateado,
...
// Función para obtener la imagen como data URL
